...
<script>
        $("form").submit(function(){
          console.log("aasas");
            ...
            axios.post("{{route('adminLogin')}}",new FormData($("form")[0])).then(response=>{
                console.log(response);
                var data = response.data;
                if(data.success) notify(null,'Logged in successfully!!','top right','success',true,"{{route('home')}}",1000);
                else{
                    for (var a in data['error']['message']) {
                        console.log(a);
                        notify('input[name='+a+']',data['error']['message'][a][0], 'botton left');
                    }
                }
            }).catch(error=>{
                notify(null,'Something Went wrong', 'botton left');
                console.log(error);
            });
        });
    </script>